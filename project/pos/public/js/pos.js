$(document).ready( function(){
	...
	.autocomplete( "instance" )._renderItem = function( ul, item ) {
		if(item.enable_stock == 1 && item.qty_available <= 0){
        	...
        	string += ' (' + item.sub_sku + ')' + "<br> Precio: " + selling_price + ' (Sin existencias) </li>';
        	...
        	string += ' (' + item.sub_sku + ')' + "<br> Precio: " + selling_price + " Disponibles: " + Math.trunc(item.qty_available) + "</div>";
    		return $( "<li>" )
        		.append(string)
        		.appendTo( ul );
        }
	...
	$('table#pos_table tbody').on('change', 'input.pos_quantity', function(){
		...
		var unit_price_inc_tax = __read_number(tr.find('input.pos_basic_price_sell'));
		var pos_unit_price = __read_number(tr.find('input.pos_unit_price'));
		var line_total = entered_qty * unit_price_inc_tax;
		
		var purchase_it = __read_number(tr.find('input.pos_basic_price'));
		var sell_it = __read_number(tr.find('input.pos_basic_price_sell'));
		
		var limit1 = __read_number(tr.find('input.pos_li'));
		var pcntg1 = __read_number(tr.find('input.pos_lp'));
		var limit2 = __read_number(tr.find('input.pos_li2'));
		var pcntg2 = __read_number(tr.find('input.pos_lp2'));
		var limit3 = __read_number(tr.find('input.pos_li3'));
		var pcntg3 = __read_number(tr.find('input.pos_lp3'));
		var limit4 = __read_number(tr.find('input.pos_li4'));
		var pcntg4 = __read_number(tr.find('input.pos_lp4'));
		var limit5 = 9999999;
		var pcntg5 = __read_number(tr.find('input.pos_lp4'));
		var price_unit = 0;
		
        if (entered_qty <= limit1){
		    line_total = (entered_qty * unit_price_inc_tax)*1;
		    price_unit = unit_price_inc_tax;
		}
		...
		if (entered_qty >= limit1){
		    ...
		    price_unit = unit_price_inc_tax;
		        ...
		        ptg1 =  ((((purchase_it)/100) * pcntg1));
		        price_c = purchase_it + ptg1;
		        line_total=  entered_qty * price_c ;
		        price_unit =price_c;
		        unit_price_inc_tax = price_unit;
		    }
		    
		}
		if (entered_qty >= limit2){
		    if (limit2 === 0){    
		    line_total = (entered_qty * unit_price_inc_tax)*1;
		    price_unit = unit_price_inc_tax;
		    }
		    else{
		        ptg2 =  ((((purchase_it)/100) * pcntg2));
		        price_c2 = purchase_it + ptg2;
		        line_total=  entered_qty * price_c2 ;
		        price_unit =price_c2;
		        unit_price_inc_tax = price_unit;
		    }
		    
		}
		if (entered_qty >= limit3){
		    if (limit3 === 0){    
		    line_total = (entered_qty * unit_price_inc_tax)*1;
		    price_unit = unit_price_inc_tax;
		    }
		    else{
		        ptg3 =  ((((purchase_it)/100) * pcntg3));
		        price_c3 = purchase_it + ptg3;
		        line_total=  entered_qty * price_c3 ;
		        price_unit =price_c3;
		        unit_price_inc_tax = price_unit;
		    }
		    
		}
		if (entered_qty >= limit4){
		    if (limit4 === 0){    
		    line_total = (entered_qty * unit_price_inc_tax)*1;
		    price_unit = unit_price_inc_tax;
		    }
		    else{
		        ptg4 =  ((((purchase_it)/100) * pcntg4));
		        price_c4 = purchase_it + ptg4;
		        line_total=  entered_qty * price_c4 ;
		        price_unit =price_c4;
		        unit_price_inc_tax = price_unit;
		    }
		...
		tr.find('span.price_basic').text(__currency_trans_from_en(unit_price_inc_tax, true));
		tr.find('span.pqs_s').text(pos_unit_price, true);
		
		__write_number(tr.find('input.pos_unit_price'), price_unit);
		__write_number(tr.find('input.pos_unit_price_inc_tax'), price_unit);
		
		document.getElementById('descount').value = document.getElementById('pba').innerHTML;
		pos_total_row();
	});
	...
	$('button#posShippingModalUpdate').click(function(){
		...
		//Password detect
        var pass_compile = __read_number($('input#pasword_ship'));
...
function pos_total_row(){
	...
	var view_sendch =  __read_number($('input#shipping_charges'));
	var pass_ship = __read_number($('input#password_ship'));
	var pin_ship = __read_number($('input#pin_id'));

	
	if(view_sendch > 0){
	    if(pass_ship == pin_ship){
    	var view_ship_charge = view_sendch;
	    }
	    else{
    	var view_ship_charge = 0;
	    }
	}
	else
	{
	...
	$('span#shipping_charges_amount').text(view_ship_charge);
...
function calculate_billing_details(price_total){
	...
	var sendch =  __read_number($('input#shipping_charges'));
	
	var pasw_ship = __read_number($('input#password_ship'));
	var piw_ship = __read_number($('input#pin_id'));
	
	if(sendch > 0){
	    if(pasw_ship == piw_ship){
	    var shipping_charges = sendch;}
	    else{
...
function pos_discount(total_amount){
    ...
    var sellds =__calculate_amount(calculation_type, calculation_amount, total_amount);
    var pass = __read_number($('input#password_desc'));
    var pin = __read_number($('input#pin_id'));
