...
$Festivos= "SELECT * FROM feriados";
	...
	$FQ -> bindParam(':eid',$eid, PDO::PARAM_STR);
	$FQ -> execute();
...
$mxday= array(null,"31","29","31","30","31","30","31","31","30","31","30","31");
$AñoCurso = date('Y');
$DiasAño=date('z',  mktime(0,0,0,12,31,$AñoCurso+1));
for ($m=1; $m<=12;$m++){
    $mu=$m;
    if($m<=9){ $mu= "0".$m;}
    $MesCurso=$mxday[$m];
...
for($i = 1; $i <= $MesCurso; $i++) {
        ...
        if(!in_array($fechrt,$Carga)){
            ...
            $QRYconsulta->bindParam(':user',$eid,PDO::PARAM_STR); 
            $QRYconsulta->execute();
...
$zDay = date('t');                                            
$zdL=0;
for($pYea = 0; $pYea <= 365; $pYea++) {
$zPE=0;
$zPS=0;
$zdata = "SELECT * FROM tblogin WHERE empid=:user AND Año=:year AND dia=:pYea";
$zqry = $dbh -> prepare($zdata);
$zqry->bindParam(':user',$eid,PDO::PARAM_STR);
$zqry->bindParam(':year',date("Y"),PDO::PARAM_STR);
$zqry->bindParam(':pYea',$pYea,PDO::PARAM_STR);
$zqry->execute();
$zQres=$zqry->fetchAll(PDO::FETCH_OBJ);
    if($zqry->rowCount() > 0){
        foreach($zQres as $zQr){
            $zCmp=$zQr->reg_type;
                if($zCmp=='ENTRADA'){$zPE=1;}
                if($zCmp=='SALIDA'){$zPS=1;}
        }
    }$zPT=$zPE+$zPS;
    if($zPT==2){$zdL++;}
} 
	...
	$sql = "SELECT *  FROM tblogin WHERE empid=:eid AND Año=:year ORDER by Entrada DESC";
	...
	$objPHPExcel->getActiveSheet()->setCellValue('E2', 'Dias Laborables del ' . $AñoCurso . ": " . $c ." / ".$DiasAño);
...
foreach($results as $result)
{
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('E3', "Dias laborados:  $zdL");
    $pctg=$PTm/$c;
	...
	header("Content-Disposition: attachment;filename= $clve:$toyear:$name.xlsx");
