					...
					<tr>
						...
						<?php
                                ...
                                if($QyBar->rowCount() > 0){  
                                        foreach($RsBar as $RB){
                                            $LocPin=$RB -> pin;
                                            $pin = base64_decode($LocPin); 
                               ?>
                               
                               <div class="col-md-6 hidden">
			        	        <div class="form-group">
			        	            {!! Form::label("pin_id$cnt_x", 'Password' . ':*' ) !!}
			        	            <div class="input-group">
			        	                {!! Form::text("pin_id$cnt_x", "$pin", ['class' => 'form-control', 'readonly']); !!}
			        	            </div>
			        	        </div>
			        	        </div>
                               
                               <?php    //proximamente $cnt_x++;         
                                        }
                                    }
					...
					<tr>
						<td>
							<div class="col-sm-2 col-xs-6 col-2px-padding">
                                <!-- col-sm-2 col-xs-6 col-2px-padding
								...
								-->
								<?php 
								    $id_send = request()->session()->get('user.id');
								    $id_send=base64_encode($id_send);
								    $id_buss = request()->session()->get('business.id');
								    $id_buss=base64_encode($id_buss);
								?>
								<a href="../public/util/?c=<?php echo $id_send;?>" target="popup">
								    
								    <button type="button" 
									class="btn bg-orange btn-block btn-flat" 
									id="devoluciones">
								...
								<a href="../public/util/psh/?c=<?php echo $id_send; ?>&b=<?php echo $id_buss; ?>" target="popup">
								...
								<a href="../public/util/retiro/funct/?c=<?php echo $id_send; ?>&b=<?php echo $id_buss; ?>" target="popup">
								    
								    <button type="button" 
									class="btn bg-blue btn-block btn-flat" 
									id="psh">
